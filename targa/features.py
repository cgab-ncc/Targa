...
def __compute_features(gene_expression_data, stage_data, method, gene):
    ...
    # f3. Stage_4_Tumor_Stat_Summary
    if method == Features.BuildMethods.AVERAGE:
        f3 = np.mean(stage_4_tumor_expr)
    if method == Features.BuildMethods.MEDIAN:
        f3 = np.median(stage_4_tumor_expr)

    # f4. Stage_3_Tumor_Stat_Summary
    if method == Features.BuildMethods.AVERAGE:
        f4 = np.mean(stage_3_tumor_expr)
    if method == Features.BuildMethods.MEDIAN:
        f4 = np.median(stage_3_tumor_expr)

    # f5. Stage_2_Tumor_Stat_Summary
    if method == Features.BuildMethods.AVERAGE:
        f5 = np.mean(stage_2_tumor_expr)
    if method == Features.BuildMethods.MEDIAN:
        f5 = np.median(stage_2_tumor_expr)
...
def build_late_stage_features(gene_expression_data, genes, stage_data, method=Features.BuildMethods.MEDIAN, verbose=True):
    ...
    EFFECTS:    computes 5 features (see below) based on the specified method,
                ...
                f1 = Stage_4_Tumor_Stat_Summary - Stage_3_Tumor_Stat_Summary
                f2 = Stage_3_Tumor_Stat_Summary - Stage_2_Tumor_Stat_Summary
                f3 = Stage_4_Tumor_Stat_Summary - Stage_4_Normal_Stat_Summary
                f4 = Stage_3_Tumor_Stat_Summary - Stage_3_Normal_Stat_Summary
                f5 = Stage_2_Tumor_Stat_Summary - Stage_2_Normal_Stat_Summary
    """
    ...
    i = 1
    for gene in genes:
        f1, f2, f3, f4, f5 = __compute_features(gene_expression_data=gene_expression_data,
